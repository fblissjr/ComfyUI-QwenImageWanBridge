        ...
        return {
            ...
            "optional": {
                ...
                "upscale_method": (["nearest-exact", "bilinear", "area", "bicubic", "lanczos"], {
                    "default": "bicubic",
                    "tooltip": "Interpolation method for resizing images to match the first image's resolution."
                }),
            }
        }
...
• All images are resized to match the dimensions of image1.
• 'concat' creates a side-by-side image.
• 'grid' creates a 2x2 image layout.
...
The output is a single image tensor, ready for the QwenVLTextEncoder."""
    ...
    def create_composite_canvas(self, reference_method: str,
        ...
        # Step 1: Standardize all images to the resolution of the first image
        # This ensures all parts of the canvas have consistent dimensions.
        target_h, target_w = images[0].shape[1:3]
        logger.info(f"[Multi-Reference] Standardizing all images to {target_w}x{target_h}.")

        standardized_images = [images[0]]
        for img in images[1:]:
            # Use 'stretch' logic to force all images to the same dimensions
        ...
        # Step 2: Create the composite canvas based on the chosen method
